...
v10 positions PTO‑WSP as a **general DL programming/compilation framework**. The spec below therefore focuses on portable
semantic contracts (workload–schedule, dispatch/task_window, CSP/CSPT) and on a compilation pipeline that remains
backend-extensible.
...
- Backends provide kernel cycle reports:
  - pto-runtime `a2a3sim`/`a2a3`: PTO‑ISA cycle reports (or an equivalent runtime-defined cycle counter with a published mapping)
...
### 1.4 Python-driven compilation pipeline (AST + optional MLIR islands)

v10 treats Python as the **authoring and compilation driver**, not the execution engine.

Requirements:

- Kernel/orchestration authoring is expressed in Python, lowered via a **source-AST pass pipeline** into typed IR and then into
  backend artifacts.
- “Kernel bodies” are modeled as **backend-specific intrinsics**:
  - a typed builtin/intrinsic registry exists at the Python level,
  - backends provide emitter(s) that lower intrinsics into C++ (and then into backend toolchains).
- The compiler may optionally route a *pass-selected* region through a more powerful internal IR pipeline:
  - `Python AST → MLIR → C++/artifact`
  - region selection is decided dynamically by compiler passes (no user-facing markers required).
  - constraints (e.g., “fully typed boundary”) are optional heuristics, not hard requirements.

Non-negotiable:
- regardless of backend target, v10 must **codegen C++ and build an executable artifact** for execution; “reduce Python↔C++ bridge”
  is a compilation-pipeline goal, not a target-backend choice.
...
`dispatch` maps tasks (kernel invocations) to **scheduler domains**.

In v10, `dispatch(policy)` is about **which AICPU scheduler (or scheduler queue)** is responsible for scheduling a task
into workers (AICore/AIV/AIC, etc.). This is **not** a per-task `core_type` selection mechanism.
...
#### 3.1.1 Required multi-scheduler semantics (v10)

Backends that claim `dispatch` enforcement must support **multiple scheduler domains** (conceptually multiple AICPU
schedulers / ready-queue partitions) and must allow the artifact to assign each task to a domain.

v10 requires two complementary assignment paths:

1) **Static fast-path:** codegen computes `scheduler_id` for each task from compile-time known axes/bindings/symbols and
   serializes it into the task descriptor/payload consumed by the runtime.
2) **Dynamic hook-path:** the orchestrator/runtime can invoke a `dispatch_hook(policy_id, task_ctx, runtime_ctx)` to choose
   `scheduler_id` at runtime when mapping depends on runtime state (symbols, counters, adaptive policies).

The capability matrix must declare whether the backend supports:
- static assignment only,
- dynamic hook only, or
- both (required for “v10 complete” backends).
...
### 6.1 pto_runtime_a2a3sim (reference simulation backend)

- runnable locally without Ascend toolchains
- serves as the **reference correctness backend** for v10 semantics validation
- must preserve v10 CSP + dispatch semantics (including multi-scheduler `dispatch`)
- implemented by targeting `pto-runtime` **`a2a3sim`** (host runtime + simulated AICPU scheduler + simulated AICore workers)

### 6.2 pto_runtime_a2a3
...
## 7. pto-runtime integration contract (Ascend + a2a3sim)
