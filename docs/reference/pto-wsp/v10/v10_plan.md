...
> **Baseline:** v9.3 design / 0.1.0 implementation (CPU-sim artifact semantics; v10 shifts reference simulation to `pto_runtime_a2a3sim`)
...
v10 focuses on turning PTO‑WSP into a **general, scalable, extensible DL programming/compilation framework** and a **mature
multi-backend workload+schedule compiler**, while **hardening and completing** the v9 headline features (CSP +
dispatch/task_window-based scheduling).

Architecturally, v10 is explicitly decoupled into:
- **DSL (Python)**: workload–schedule as the portable surface + backend-specific intrinsic programming
- **Compilation**: source-AST pass infrastructure + optional internal AST↔MLIR bridge for pass-selected regions
- **Emitter/Backend**: backend registry + artifact packaging + toolchain build/run integration

Concretely, v10 targets:
...
v10 treats `pto_runtime_a2a3sim` as the **reference correctness backend** for schedule/CSP semantics validation. Native `cpu_sim`
is considered a v9 legacy backend and is planned to be removed during v10 implementation, but only after a parity gate:

- all v9 semantics (CSP/CSPT, `task_window` stall-only backpressure, dynamic symbols/slots, dispatch semantics) must be validated
  on `pto_runtime_a2a3sim` first (including multi-AICPU scheduler-domain semantics for `dispatch(policy)` as defined in v10).
...
- **Backend maturity**
  - Make the “artifact runtime” architecture clearly layered and testable:
    - frontend → typed IR → codegen (pto_runtime_* / AIE) → artifact runtime
...
- **CSP as a complete feature (no compromise)**
  ...
  - Backend applicability (v10 target):
    - `pto_runtime_a2a3sim`: runnable and fully validated (baseline)
...
- A versioned “compiler → backend” surface for kernel/orchestration packaging:
  - backend-extensible artifact packaging (kernels + orchestration + metadata)
  - backend emitter registry for intrinsic codegen
  - optional `AST ↔ MLIR` pipeline hook for pass-selected regions

Acceptance criteria:
- `pto_runtime_a2a3sim`: examples continue to be fully self-validating (golden + runner) and pass.
- Runtime semantics are explicitly described and match implementation.

### W2) True CSP/CSPT semantics (cross-backend; a2a3sim first)
...
`pto_runtime_a2a3sim` remains the correctness anchor:
- Every `examples/*/*_example.py` must:
  - generate artifacts
  - run `pto_runtime_a2a3sim`
...
See:
- `docs/reference/pto_runtime/analysis.md`
- `docs/reference/pto_runtime/integration.md`
